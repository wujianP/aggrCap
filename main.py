...
def main(args):
    ...
    for cur_iter, (filenames, prompts, captions, actions) in enumerate(data_loader):
        ...
        new_prompts = [
            f"Input: These are captions of the frames in temporal order within the same video: {cp}. please summarize the whole video according to the frame captions in short. Always answer in one short sentence. Output:"
            for cp in captions
        ]
